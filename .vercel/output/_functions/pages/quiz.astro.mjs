      ...
      try {
        ...
        if (variantInfo) {
          ...
          if (window.trackImpression) {
            ...
            console.error('trackImpression function not available on window');
          }
        } else {
          console.log('No A/B test variant info found in sessionStorage');
        }
      } catch (error) {
        console.error('Error in A/B test tracking:', error);
      }
    });
...
Back to Homepage
  ...
  <\/script> `])), maybeRenderHead(), JSON.stringify(quizQuestions), quizQuestions.map((question, index) => renderTemplate`<div class="mb-6 bg-white dark:bg-slate-700 rounded-lg"> <div class="p-6"> <p class="text-lg font-medium text-gray-800 dark:text-white mb-4">${index + 1}. ${question.text}</p> </div> <div class="flex justify-between items-center w-full flex-nowrap gap-6"> ${["Strongly Disagree", "Disagree", "Neutral", "Agree", "Strongly Agree"].map((option, optionIndex) => renderTemplate`<div class="flex flex-col items-center"> <div class="flex items-center"> <input type="radio"${addAttribute(`q${index}-opt${optionIndex + 1}`, "id")}${addAttribute(`question-${index}`, "name")}${addAttribute(optionIndex + 1, "value")} class="h-4 w-4 text-orange-500 border-gray-300 focus:ring-orange-500 dark:bg-slate-700 dark:border-slate-600" required onclick="updateQuizProgress()"> </div> <label${addAttribute(`q${index}-opt${optionIndex + 1}`, "for")} class="mt-1 text-sm text-center text-gray-700 dark:text-gray-300"> ${option} </label> </div>`)} </div> </div>`)) })}`;
}, "C:/Dev/jason_personal_website/src/pages/quiz.astro", void 0);

const $$file = "C:/Dev/jason_personal_website/src/pages/quiz.astro";
const $$url = "/quiz";
