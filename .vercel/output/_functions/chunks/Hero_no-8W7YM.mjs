import { c as createAstro, a as createComponent, b as renderTemplate, r as renderComponent, m as maybeRenderHead } from './astro/server_DgPtluSo.mjs';
import 'kleur/colors';
import { $ as $$Image } from './_astro_assets_CLKYWX5i.mjs';
import imgHeroMain from './jj_5__eLixI.mjs';
...
const $$Hero = createComponent(async ($$result, $$props, $$slots) => {
  ...
  Astro2.self = $$Hero;
  const {
    image = { src: imgHeroMain, alt: "Default alt text" },
  ...
  return renderTemplate(_a || (_a = __template(["", '<section class="relative overflow-hidden py-24 md:py-32">  ', '  <div class="absolute inset-0 w-full h-full bg-gray-900 opacity-60 -z-10"></div>  <div class="container mx-auto px-4 relative z-10 text-center"> <div class="max-w-3xl mx-auto"> <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight drop-shadow-md"> ', ' </h1> <p class="text-lg text-gray-200 mb-8 drop-shadow-sm"> ', ` </p> <form id="hero-page-form" class="flex flex-col sm:flex-row gap-4 justify-center max-w-xl mx-auto"> <input type="hidden" name="ab_test_variant" value="control"> <input type="hidden" name="signup_source" value="hero"> <input type="hidden" name="ab_user_identifier" id="ab-user-identifier-input" value=""> <label for="hero-email-bg" class="sr-only">Email address</label> <input type="email" id="hero-email-bg" name="email" placeholder="your.email@example.com" required class="w-full sm:w-auto flex-grow px-4 py-3 rounded-md border border-gray-300 dark:border-gray-600 bg-white/90 dark:bg-gray-800/90 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:outline-none placeholder-gray-500 dark:placeholder-gray-400"> <button type="submit" class="px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white text-lg font-semibold rounded-full shadow-md transition duration-300 ease-in-out transform hover:scale-105 whitespace-nowrap">
Get Started Now
</button> </form> <p id="hero-page-form-message" class="text-sm text-gray-300 mt-4 drop-shadow-sm"></p> </div> </div> </section> <script>
...
<\/script>`])), maybeRenderHead(), renderComponent($$result, "Image", $$Image, { "src": image.src, "alt": image.alt, "width": 1920, "height": 1080, "format": "webp", "quality": 75, "loading": "eager", "class": "absolute inset-0 w-full h-full object-cover -z-20" }), headline, subheadline);
}, "C:/Dev/jason_personal_website/src/components/widgets/Hero.astro", void 0);
